...
token_string_table := [Token_Kind_B]string{
        .Invalid = "invalid",
        .EOF = "EOF",
        .Using = "using",
...
tokenizer_init :: proc(t: ^Tokenizer_B, filename, data: string) {
    t.data = data
    t.filename = filename
    t.line = 1
    t.column = 0
...
get_next_token :: proc(t: ^Tokenizer_B) -> (token: Token_B, err: Error) {
    advance :: proc(t: ^Tokenizer_B, loc := #caller_location) {
        t.offset += 1
        t.column += 1
    ...
    is_whitespace :: proc(t: ^Tokenizer_B) -> bool {
        c := t.data[t.offset]
        return c == ' ' || c == '\t' || c == '\r' || c == '\n'
    }

    skip_whitespace :: proc(t: ^Tokenizer_B) {
        old_offset := t.offset
    ...
    forward_word :: proc(t: ^Tokenizer_B) {
        loop: for t.offset < len(t.data) {
            if is_whitespace(t) { break loop }
            advance(t)
        }
...
string_to_token_kind :: proc(str: string) -> (kind: Token_Kind_B) {
    kind = .Word
...
token_to_string :: proc(token: Token_B) -> string {
    if token.kind == .Word {
